...
spark = (SparkSession.builder
    ...
    .config("spark.driver.memory", "2560m")  # 适当增加，以处理少量聚合和文件操作
...
def predict_public_opinion_trend(sentiment_predicted_df):
    ...
    # 如果 Spark 3.0+，datediff可以直接用日期列和字符串日期
    # 如果 Spark 版本较低，可能需要udf来处理datediff(col, lit)
    ...
    # 原来的 id 列 (monotonically_increasing_id) 可以保留，但我们主要使用 days_since_start
    # daily_pivot_df = daily_pivot_df.withColumn("id", monotonically_increasing_id()) # 这行可以暂时注释掉或删除

    # 修改 assembler 的 inputCols，使用新的 'days_since_start'
    ...
    # 修改 spark.createDataFrame 的结构，因为我们现在只有 created_date 和 days_since_start
